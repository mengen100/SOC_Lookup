        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
          <div className="min-w-0">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link className="rounded bg-accent px-4 py-2 text-sm font-semibold text-white focus-visible:outline-accent" href="/windows-events/">
                Browse Windows Events
              </Link>
              <Link className="rounded border border-line bg-white px-4 py-2 text-sm font-semibold text-ink focus-visible:outline-accent" href="/sysmon-events/">
                Browse Sysmon Events
              </Link>
            </div>
          </div>
          <div className="grid min-w-0 grid-cols-3 gap-3 rounded border border-line bg-white p-4 lg:mt-7">
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      ...
      <p className="mt-1 break-words text-xs text-steel">{label}</p>
    </div>
  );
}
